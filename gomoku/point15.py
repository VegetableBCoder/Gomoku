class Point:
    def __init__(self,record,x,y,color):
        self.record=[[0 for i in range(15)]for j in range(15)]
        for i in range(15):
            for j in range(15):
    ...
    def moveWithDirection(self,dir,dis):#参数为方向和距离
        ...
        if xx<0 or xx>=15 or yy<0 or yy>=15:
            return [3,3,3]
        ans[0]=self.record[xx][yy]
        if xxx<0 or xxx>=15 or yyy<0 or yyy>=15:
            return [ans[0],3,3]
        ans[1]=self.record[xxx][yyy]
        if xxxx<0 or xxxx>=15 or yyyy<0 or yyyy>=15:
