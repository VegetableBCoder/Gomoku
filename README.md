...
* 输入: 15*15*2双通道  
  ...
  - 通道1: 对方棋子
* 输出: 价值头, 策略头
  - 价值头: 3维向量, 表示胜负平的logits
  - 策略头: 225维向量, 表示每个点的选点logits
* 通道/维度: 192
* 残差块: 每个块2层卷积; x -> Conv -> BN -> ReLU -> Conv -> BN -> + x -> ReLU
* Res块数: 12, 网络层数 12*2+1=25
* 卷积核: 3*3
* 输入->首层:
  * 升维: 2 -> 192
  * 192个维度分别从双通道15*15的每个点为中心周围3*3的范围内做数据做卷积
  * 输入双通道就是3*3*2, 18个数值 按照当前通道的按权重求和 得到一个值, 共15*15个值
  * 192个通道就是192*15*15, 然后 BN-> ReLU
* 隐含层:  类似于输入到首层, 只不过每个通道计算的来源是前置输入的192个通道的数据, 仍然保持192*15*15的输出不变
* 隐含层->策略头:
  * 降维, 从192维降到1维
  * 对15*15上的每个i,j点计算192个维度的加权求和, 最终保持15*15不变但降为1维
  * 展平成225个logits
* 隐含层->价值头:
  * 先对每个通道15*15加权平均池化,  然后展平, 得到192个通道每个一个值
  * 然后走全连接层, 但降为128维(Linear(192, 128)), 然后ReLU
  * 再次走全连接层(Linear(128, 3)) 最终降为3维向量, 得到价值头
